...
namespace Interfaces.FFmpeg
{
    ...
    class CameraM3U8 : SourceFormater
    {
                ...
                try
                {
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        unsafe
                        {
                            ...
                            AVInputFormat* ifmt = ffmpeg.av_find_input_format("hls");
                            int ret = ffmpeg.avformat_open_input(&fmtCtx, segmentUrl, ifmt, &options);
                            if (ret < 0)
                            {
                                var buffer = stackalloc byte[1024];
                                ffmpeg.av_strerror(ret, buffer, 1024);
                                string errMsg = Marshal.PtrToStringAnsi((IntPtr)buffer);
                                Console.WriteLine($"Error opening input: {errMsg}");
                                continue;
                            }
